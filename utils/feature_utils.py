...
def create_features_from_raw_df(input_df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = X.drop('C_vl_protein_sequence', axis=1)
    X = X.drop('C_vh_protein_sequence', axis=1)
    X = X.drop('heavy_aligned_aho_length', axis=1)
    X = X.drop('light_aligned_aho_length', axis=1)
